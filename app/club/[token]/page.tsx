...
export default function ClubScannerPage({ params }: { params: { token: string } }) {
  ...
  useEffect(() => { getClubByToken(token).then(setClub); }, [token]);
  const onScan = useCallback(async (qrToken: string) => { if (!club) return; try { const student = await getStudentByToken(qrToken); if (!student) { setStatus({ tone: "error", title: "ไม่พบผู้เข้าร่วม", detail: "กรุณาลงทะเบียนก่อนรับแสตมป์" }); return; } const result = await recordStamp(student.id, club.id); setStatus(result.created ? { tone: "success", title: `ประทับตรา ${club.name} สำเร็จ`, detail: `รหัสนักศึกษา ${student.studentCode}` } : { tone: "warning", title: "เคยรับแสตมป์แล้ว", detail: `${club.name} • รหัส ${student.studentCode}` }); } catch { setStatus({ tone: "error", title: "บันทึกไม่สำเร็จ", detail: "กรุณาลองสแกนอีกครั้ง" }); } }, [club]);
