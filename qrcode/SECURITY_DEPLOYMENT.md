...
1. Apply both login-security migrations in order:
   - `supabase/migrations/202608080001_server_side_login_protection.sql`
   - `supabase/migrations/202608080002_participant_registration_protection.sql`
...
The second migration removes anonymous access to participant registration. Deploy the matching `/api/register/participant` route at the same time so registration remains available.
