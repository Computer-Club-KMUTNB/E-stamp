...
export default function RewardScannerPage({ params }: { params: { token: string } }) {
  ...
  useEffect(() => { getRewardBoothByToken(token).then(async (value) => { setBooth(value); if (value) setClubs(await getClubsByLocation(value.location)); }); }, [token]);
  const onScan = useCallback(async (qrToken: string) => { if (!booth) return; try { const student = await getStudentByToken(qrToken); if (!student) { setStatus({ tone: "error", title: "ไม่พบผู้เข้าร่วม" }); return; } const stamps = await getStampsForStudent(student.id); const stampIds = new Set(stamps.map((stamp) => stamp.clubId)); const missing = clubs.filter((club) => !stampIds.has(club.id)); if (missing.length) { setStatus({ tone: "error", title: `ขาดอีก ${missing.length} ชมรม`, detail: missing.map((club) => club.name).join(", ") }); return; } const oldClaim = await getRewardClaim(student.id, booth.location); if (oldClaim) { setStatus({ tone: "warning", title: "รับรางวัลที่จุดนี้แล้ว", detail: "ไม่สามารถรับรางวัลซ้ำได้" }); return; } await createRewardClaim(student.id, booth.location); setStatus({ tone: "success", title: "รับรางวัลได้", detail: `🎉 แสตมป์ครบแล้ว • รหัส ${student.studentCode}` }); } catch { setStatus({ tone: "error", title: "ตรวจสอบไม่สำเร็จ", detail: "กรุณาลองสแกนอีกครั้ง" }); } }, [booth, clubs]);
  ...
  return <div className="mx-auto max-w-4xl py-4 sm:py-8"><header className="mb-5 text-center"><p className="font-bold text-brand">จุดตรวจรับรางวัล</p><h1 className="mt-1 text-3xl font-black sm:text-5xl">{locationNames[booth.location]}</h1><p className="mt-2 text-slate-600">ต้องมีแสตมป์ครบ {clubs.length} ชมรม</p></header><section className="card">{status ? <div className="space-y-5"><StatusBox status={status} /><button className="primary w-full" onClick={() => { setStatus(null); setResetKey((n) => n + 1); }}>สแกนคนถัดไป</button></div> : <Scanner onScan={onScan} resetKey={resetKey} />}</section></div>;
}
