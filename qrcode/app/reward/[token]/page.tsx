...
export default function RewardScannerPage({ params }: { params: { token: string } }) {
  ...
  return <div className="mx-auto max-w-4xl py-4 sm:py-8"><header className="mb-5 text-center"><p className="font-bold text-brand">จุดตรวจรับรางวัล • {locationNames[booth.location]}</p><h1 className="mt-1 text-3xl font-black sm:text-5xl">ตรวจแสตมป์ครบทุกบูธ</h1><p className="mt-2 text-slate-600">ต้องมีแสตมป์ครบ {clubs.length} บูธ ทั้งโซนหน้าและโซนหลัง</p></header><section className="card">{status ? <div className="space-y-5"><StatusBox status={status} /><button className="primary w-full" onClick={() => { setStatus(null); setResetKey((n) => n + 1); }}>สแกนคนถัดไป</button></div> : <Scanner onScan={onScan} resetKey={resetKey} />}</section></div>;
}
