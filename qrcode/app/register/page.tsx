...
export default function RegisterPage() {
  const [code, setCode] = useState(""); const [name, setName] = useState(""); const [student, setStudent] = useState<Student | null>(null); const [error, setError] = useState(""); const [loading, setLoading] = useState(false);
  async function submit(event: FormEvent) { event.preventDefault(); if (!/^\d{13}$/.test(code)) { setError("กรุณากรอกรหัสนักศึกษาเป็นตัวเลข 13 หลัก"); return; } if (name.trim().length < 2) { setError("กรุณากรอกชื่อผู้เข้าร่วม"); return; } setLoading(true); setError(""); try { setStudent(await createStudent(code, name)); } catch (caught) { setError(caught instanceof Error ? caught.message : "เกิดข้อผิดพลาด กรุณาลองใหม่อีกครั้ง"); } finally { setLoading(false); } }
  return <div className="mx-auto max-w-2xl py-6 sm:py-12"><section className="card">
    ...
    {!student ? <form className="mt-8 space-y-5" onSubmit={submit}><div><label className="block text-lg font-bold" htmlFor="name">ชื่อ–นามสกุล</label><input id="name" autoComplete="name" maxLength={120} value={name} onChange={(e) => setName(e.target.value)} className="mt-3 min-h-16 w-full rounded-2xl border-2 border-slate-300 px-5 text-xl outline-none focus:border-brand focus:ring-4 focus:ring-indigo-100" placeholder="ชื่อผู้เข้าร่วมงาน" /></div><div><label className="block text-lg font-bold" htmlFor="studentCode">รหัสนักศึกษา 13 หลัก</label><input id="studentCode" inputMode="numeric" autoComplete="off" maxLength={13} value={code} onChange={(e) => setCode(e.target.value.replace(/\D/g, ""))} className="mt-3 min-h-16 w-full rounded-2xl border-2 border-slate-300 px-5 text-xl outline-none focus:border-brand focus:ring-4 focus:ring-indigo-100" placeholder="เช่น 6601234567890" /></div>
